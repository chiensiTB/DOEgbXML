...
namespace DOEgbXML
{
    public class gbXMLSpaces
    {
        ...
        public static gbXMLSerializer.powerPerAreaUnitEnum getPowerEnum(string gbXMLstring)
        {
                ...
                foreach (XmlNode spaceNode in nodes)
                {
                    ...
                    if (spaceNode.HasChildNodes)
                    {
                        ...
                        foreach (XmlNode node in childNodes)
                        {
                            ...
                            else if (node.Name == "LightPowerPerArea")
                            {
                                space.slp = new LightPowerPerArea();
                                XmlAttributeCollection lpatts = node.Attributes;
                                foreach (XmlAttribute at in lpatts)
                                {
                                    if (at.Name == "unit")
                                    {
                                        space.slp.unit = getPowerEnum(at.Value);
                                    }
                                }
                                space.slp.lpd = node.InnerText;
                            }
                            else if (node.Name == "EquipPowerPerArea")
                            {
                                space.sep = new EquipPowerPerArea();
                                XmlAttributeCollection epatts = node.Attributes;
                                foreach (XmlAttribute at in epatts)
                                {
                                    if (at.Name == "unit")
                                    {
                                        space.sep.unit = getPowerEnum(at.Value);
                                    }
                                }
